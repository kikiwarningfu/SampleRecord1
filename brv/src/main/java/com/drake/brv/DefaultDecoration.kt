 ...
 * 9. 支持分组条目的分割线
 *
 * @property startVisible 在[GridLayoutManager]/[StaggeredGridLayoutManager]中控制上下是否显示分割线,
 * 在[LinearLayoutManager]中控制顶部是否显示分割线
 * @property endVisible 在[GridLayoutManager]/[StaggeredGridLayoutManager]中控制左右是否显示分割线,
 * 在[LinearLayoutManager]中控制底部是否显示分割线
 * @property expandVisible 控制[ItemExpand.itemExpand]为true的情况下是否显示分割线, 但当你配置[onEnabled]
 * 后则无效, 因为此字段为其默认实现所用
...
//    做法是需要后台配合的，后台拿到我打包好的APK后，可以往我的APK里面写入东西，比如写一个xx.json文件。 然后这个json文件
//    ，后端可以根据需求配置不同的参数，比如做什么上线下线人的推广也可以这样做，于是就能在进入APP时读取到这个Json信息从而获
//    取到不同参数。 关于写入APK哪里，
//    我们把APK解压后可以看到有个META-INF目录，把东西写入这里就行了，对APP运行是没有多大影响的，只要你不在意增
//    加APK体积的话。 怎么读取这个目录的东西？这个就需要你去搜了，我那个项目不知道丢哪里去了，所以给不了你代码了，反正是
//    有方法获取到的。 挖坟一时爽，一直挖一直爽，啊哈哈哈哈哈哈~~~~
    /**
